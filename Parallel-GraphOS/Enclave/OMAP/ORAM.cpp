...
ORAM::ORAM(int maxSize, bytes<Key> oram_key, int instance)
: key(oram_key) {
...
void ORAM::WriteBucket(long long index, Bucket bucket) {
    ...
    ocall_write_ramStore(index, (const char*) ciphertext.data(), (size_t) ciphertext.size(), instanceNumber);
}
...
vector<Bucket> ORAM::ReadBuckets(vector<long long> indexes) {
    ...
    if (useLocalRamStore) {
        ...
        ocall_nread_ramStore(&readSize, indexes.size(), indexes.data(), tmp, indexes.size() * storeBlockSize, instanceNumber);
        for (unsigned int i = 0; i < indexes.size(); i++) {
...
void ORAM::InitializeBuckets(long long strtindex, long long endindex, Bucket bucket) {
    ...
    if (useLocalRamStore) {
        ...
        ocall_initialize_ramStore(strtindex, endindex, (const char*) ciphertext.data(), (size_t) ciphertext.size(), instanceNumber);
    }
}
        ...
        if (useLocalRamStore) {
            ...
            for (unsigned int j = 0; j <= indexes.size() / 10000; j++) {
                ...
                if (min((int) (indexes.size() - j * 10000), 10000) != 0) {
                    ...
                    ocall_nwrite_ramStore(min((int) (indexes.size() - j * 10000), 10000), indexes.data() + j * 10000, (const char*) tmp, cipherSize * min((int) (indexes.size() - j * 10000), 10000), instanceNumber);
                }
                delete tmp;
            }
        }
...
Node* ORAM::ReadWrite(Bid bid, Node* inputnode, unsigned long long lastLeaf, unsigned long long newLeaf, bool isRead, bool isDummy) {
    if (bid == 0) {
...
Node* ORAM::ReadWrite(Bid bid, Node* inputnode, unsigned long long lastLeaf, unsigned long long newLeaf, bool isRead, bool isDummy, std::array< byte_t, 16> value, bool overwrite) {
    if (bid == 0) {
        printf("bid is 0 dummy is:%d\n", isDummy ? 1 : 0);
        throw runtime_error("Node id is not set");
    }
...
void ORAM::prepareForEvictionTest() {
    /*Write 1 real Block in the test path*/
    long long leaf = 10;
    currentLeaf = leaf;
    Node* nd = new Node();
    nd->isDummy = false;
