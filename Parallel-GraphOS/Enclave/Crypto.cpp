...
KeySchedule ks_backup = KeySchedule((unsigned char *) key_data, SGX_AESGCM_KEY_SIZE);
...
void decrypt(uint8_t *ciphertext, uint32_t ciphertext_length,
        uint8_t *plaintext) {
...
void decrypt_with_aad(uint8_t *ciphertext, uint32_t ciphertext_length,
        uint8_t *plaintext,
        uint8_t *aad, uint32_t aad_len) {
...
void StreamCipher::encrypt(uint8_t *plaintext, uint32_t size) {
    ...
    copy_bytes = (uint32_t) AES_BLOCK_SIZE - leftover_plaintext_size;
    memcpy(leftover_plaintext + leftover_plaintext_size, plaintext, copy_bytes);
    ...
    uint32_t stream_enc_size = (uint32_t) ((size - copy_bytes) / AES_BLOCK_SIZE * AES_BLOCK_SIZE);
...
uint32_t StreamCipher::bytes_written() {
    return (uint32_t) (current_cipher_ptr - iv_ptr);
}
...
void StreamDecipher::decrypt(uint8_t *plaintext_ptr, uint32_t size) {
    ...
    uint32_t decrypt_bytes = (uint32_t) ((size - copied_bytes) / AES_BLOCK_SIZE * AES_BLOCK_SIZE);
    if (decrypt_bytes > 0) {
