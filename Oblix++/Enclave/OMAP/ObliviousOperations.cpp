...
void ObliviousOperations::compaction(std::vector<Node*>* data) {
    int fillPosition = 0;
    vector<int> compactionLeft;

    int size = data->size();
    for (unsigned int i = 0; i < size; i++) {
        bool cond = !Node::CTeq(Node::CTcmp((*data)[i]->evictionNode, (long long) - 1), 0);
        compactionLeft.push_back(Node::conditional_select((int) (i - fillPosition), (int) 0, cond));
        fillPosition = Node::conditional_select(fillPosition + 1, fillPosition, cond);
    }
    ...
    for (int i = 0; i < levels; i++) {
        ...
        for (unsigned int j = 0; j < size; j++) {
            bool cond = !(Node::CTeq(Node::CTcmp((*data)[j]->evictionNode, -1), 0)) && (!Node::CTeq(Node::CTcmp(compactionLeft[j], 0), 0));
            int dj = compactionLeft[j];
            int val = (dj % power);
            Node::conditional_swap(((*data)[j - val]), ((*data)[j]), cond);
            compactionLeft[j - val] = Node::conditional_select(dj - val, compactionLeft[j - val], cond);
        }
    }
